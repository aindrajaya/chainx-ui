...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/v1/exports/history?userId=${userId}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json(
        { success: false, message: data.error || 'Failed to get export history' },
